...
const BASE = (process.env.NEXT_PUBLIC_SITE_URL || 'https://findlipfillers.com').replace(/\/$/, '');
...
export default function ProvidersIndex({ providers }) {
  ...
  const metaDescription = `Browse ${providers.length} lip filler providers across ${cities.length} Colorado cities. Compare brands, pricing, and book appointments with provider options.`;

  return (
    <Layout
      title="All Lip Filler Providers in Colorado - Provider Directory"
            ...
            <div>
              {filteredAndSortedProviders.map(provider => (
                <div key={provider.id} style={{ marginBottom: '30px' }}>
                  <ProviderCard provider={provider} showEnhanced={false} trackClick={true} />
                ...
                >
                  <div style={{ flex: 1, minWidth: '250px' }}>
                    <h3 style={{ margin: '0 0 5px 0', fontSize: '1.2rem' }}>
                      <Link href={`/providers/${provider.slug}`} style={{ color: '#333', textDecoration: 'none' }}>
                        {provider.name}
                      </Link>
                    </h3>
